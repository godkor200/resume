...
export const about = [
  "5년차 백엔드 개발자 유병국입니다.",
  "커리어 전반부에는 메가프레스에서 커머스 플랫폼 백엔드를 담당했습니다. 15개+ 사이트에 하드코딩으로 운영되던 상품 페이지를 MongoDB 조합 document 기반 자동 생성 구조로 전환하고, 동시 주문 환경에서 재고 불일치 문제를 낙관적 잠금(Optimistic Lock)으로 해결하며 운영 가능한 구조를 만드는 데 집중했습니다.",
  "이후 두디스와 스페이스오디티에서 데이터 집약 서비스로 넘어갔습니다. 두디스에서는 다중 조인으로 40초 걸리던 집계 API를 비정규화 구조로 전환해 3초로 줄였고, 스페이스오디티에서는 일 평균 100만 건 K-POP 데이터 파이프라인을 설계했습니다.",
...
export const careers: Array<{
  ...
  },
  {
    ...
    achievements: [
      "메가프레스 계열 15개+ 인쇄 서비스 사이트에서 하드코딩으로 관리되던 상품 페이지를 MongoDB 조합 document 기반 자동 생성 구조로 전환 — 관리자 페이지에서 상품 정의만으로 전 사이트 상품 페이지 자동 생성, 사이트 추가 시 개발 작업 불필요",
...
export const portfolio = [
  ...
  },
  {
    ...
    achievements: [
      "메가프레스 계열 15개+ 사이트에서 하드코딩으로 운영하던 상품 상세 페이지를 MongoDB 조합 document 기반 자동 생성 방식으로 전환 — 사이트 추가·상품 수정 시 개발 작업 불필요, 운영 비용 절감",
    ...
    relatedSites: [
      { label: "프린트시티", url: "https://www.printcity.co.kr" },
      { label: "메가프린트", url: "https://www.megaprint.co.kr" },
      { label: "명함천국", url: "https://www.ecard21.co.kr" },
      { label: "말씀프린팅", url: "https://www.mspg.co.kr" },
      { label: "애드코아", url: "https://www.adcore.co.kr" },
    ],
    problem:
      "메가프레스는 15개+ 인쇄 서비스 사이트를 운영하고 있었는데, 상품 하나당 상세 페이지 하나를 각 사이트마다 직접 하드코딩하는 방식이었습니다. 사이트와 상품이 늘어날수록 페이지 수가 같이 늘었고 운영 변경이 생길 때마다 개발이 반드시 들어가야 했습니다. 레거시 파일 복사 기반 유지보수 때문에 휴먼 에러도 자주 발생했고 주문이 동시에 들어오면 재고 불일치 문제도 생겼습니다.",
    solution:
