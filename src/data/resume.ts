...
export const about = [
  ...
  "현장에서 가장 자주 마주한 문제는 조회 패턴과 맞지 않는 데이터 모델에서 비롯되는 병목과 동시성 이슈였습니다. 두디스에서는 약 40초에 달하던 집계 API 응답 시간을 약 3초로 단축했고 메가프레스에서는 재고 동시 수정으로 인한 데이터 충돌 문제를 낙관적 잠금(Optimistic Lock) 전략으로 해결했습니다.",
