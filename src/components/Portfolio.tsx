    ...
    <>
      ...
      <div className="border border-gray-200 rounded-2xl overflow-hidden hover:border-blue-200 transition-colors">
        <div className="p-6 md:p-8">
          ...
          {/* Related sites */}
          {(project as { relatedSites?: { label: string; url: string }[] }).relatedSites && (
            <div className="mb-5">
              <p className="text-xs text-gray-400 font-medium mb-2">운영 사이트 예시</p>
              <div className="flex flex-wrap gap-2">
                {(project as { relatedSites: { label: string; url: string }[] }).relatedSites.map((site) => (
                  <a
                    key={site.url}
                    href={site.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-50 border border-gray-200 rounded text-xs text-gray-500 hover:text-blue-600 hover:border-blue-200 transition-colors"
                  >
                    {site.label}
